...
**Most of this vignette is about the default, AVATAR blending**, run
over the eight public datasets described below. For each generated
subject it samples a compatible source subject’s event skeleton as a
template, then fills the covariates and endpoint trajectories with a
distance-weighted blend of similar subjects, plus subject and residual
noise. The output has the source schema, fresh identifiers, and the same
cohort size. A late section runs the model-based path on the same
theophylline data, as `scripts/demo_nlmixr2data.R` does for the first
five.
...
both `Suggests` of this package. They were not chosen for variety of
drug or disease — `synpmx` never sees the pharmacology — but for variety
of *structure*, since structure is the whole of what the algorithm reads
and the whole of what can expose a patient. “Endpoints” counts
**observation** endpoints; a compartment or `NAME` level used for dosing
is not one.
...
**Half of it is real patient data and half is not.** `warfarin`,
`nimoData`, `mavoglurant` and `pheno_sd` are real studies; `wbcSim`,
`case1_pkpd` and `mad` are model-generated; `theo_md` is real on day 1
with days 2 to 7 simulated. That distinction earns its place here for
one reason: a simulated cohort is sampled on a tidier schedule than a
real one, and the two datasets where the masking fails below —
`nimoData` and `pheno_sd` — are both real, while the one that passes
perfectly is simulated. A result on simulated data is not evidence about
a real study.
...
Enough happens by default that it is worth naming the six mechanisms
...
it. Six mechanisms act on that skeleton, and every one of them works by
*removing* something:

The article numbers them **M1 to M6**; they are listed here in the order
they run, which puts M6 between M3 and M4.
...
![](avatar-evaluation-public-data_files/figure-html/theo-schedule-map-1.png)
...
![](avatar-evaluation-public-data_files/figure-html/theo-plot-1.png)
...
![](avatar-evaluation-public-data_files/figure-html/theo-tad-1.png)
...
![](avatar-evaluation-public-data_files/figure-html/warfarin-plot-1.png)
...
**Twelve patients still have a unique schedule after coarsening, and not
one of them for a reason the grid can address.** All twelve share every
one of their observation times with somebody; what singles them out is
*which* of those shared visits they attended. Warfarin has subjects with
13, 14, 16, 17, 18, 19 and 25 observations, and the rarer counts pick
those subjects out on their own. No grid resolution helps, because the
times are already shared.

**Four of fourteen visit sets were discarded, each held by one
patient.** Those four patients stay in the cohort and keep contributing
measurements as donors — only their distinctive absences stop being
reproduced. Note that patterns discarded and patients holding them are
equal: at the default floor of 2, a pattern is discarded exactly when
one patient holds it, so the two numbers must agree. They diverge only
at a floor of 3 or more.

**About a fifth of avatars had their missed visits placed fresh.** The
...
rare to reuse. That is what those avatars get *instead of* one of the
four discarded sets; see the section on pattern strictness below.
...
![](avatar-evaluation-public-data_files/figure-html/wbc-plot-1.png)
...
**Seventeen patients have a unique schedule, split two and fifteen.**
The two with a unique observation time are the ones worth acting on:
`wbcSim` carries no nominal-time column, so the grid was inferred, and
declaring a real one would likely clear them. The fifteen with a unique
set of attended visits are past the grid’s reach.

**Five of 25 patterns were discarded.** A larger absolute number than
warfarin’s four but a smaller share — 20% against 29% — because 45
subjects on a common infusion schedule produce more repetition than 32
warfarin subjects do.
...
![](avatar-evaluation-public-data_files/figure-html/nimo-plot-1.png)
...
does. Twelve subjects hold twelve distinct visit sets; two of those are
held by a single patient and are discarded. Yet the row that gives the
game away is the next one: **every avatar’s arrangement was invented
rather than reused, 100%.** Not one avatar could be given a real
patient’s set of attended visits, so all twelve got a set built from a
*shape* — how many visits were missed, and whether the misses were
terminal, contiguous, or scattered. The shape survives; which specific
visits each patient missed does not.

**And the dose row fails outright: all twelve avatars carry a dose
schedule nobody else shares**, because the ten infusions were recorded
at actual times rather than nominal ones, so every subject’s dosing is
theirs alone. Only 7 of the 12 source regimens are represented at all.

All three failures have one cause and one fix, and `nimoData` gets that
fix demonstrated later in this vignette: constructing a nominal time
from the protocol grid takes the unique-schedule count from 12 to 5, the
invented- arrangement share from 100% to 17%, and the dose-schedule
failures from 12 to 0.
...
![](avatar-evaluation-public-data_files/figure-html/mavo-plot-1.png)
...
did most of what it could: of the 64 patients still unique afterwards,
**eleven have a unique observation time** and the other 53 share every
observation time with somebody, differing only in which of those visits
they attended.

Those 53 are the largest absolute residual in the vignette, and no grid
setting moves them. This is what the “unique visit set” row is for: it
...
The pattern rows are the reassuring part. Mavoglurant has by far the
most distinct visit sets of any dataset here — **73** — and yet only
**two** of them are held by a single patient, so only those two are
discarded and **no arrangement had to be invented at all**: for every
one of the 120 avatars, a real visit set that several patients share was
available to reuse. A large number of distinct patterns is not by itself
a problem; a large number of *singleton* patterns is.

## case1_pkpd: a study report on its own nominal grid

[`xgxr::case1_pkpd`](https://rdrr.io/pkg/xgxr/man/case1_pkpd.html) is
the only public dataset here shaped like a real study report: 180
patients across six treatment arms, a declared `NOMTIME`, two endpoints
keyed by a character `NAME` column, a baseline weight, and a `CENS`
column. Everything the nlmixr2data five leave untested — a nominal grid,
an arm to stratify on, censoring — is present.
...
**Validate the source too.** Running
[`validate_pmx()`](https://iamstein.github.io/synpmx/reference/validate_pmx.md)
on the data you are about to generate from is how a mis-declared role is
caught before it becomes a generation bug, and this dataset is the one
where it fires. Declaring `cens = "CENS"` is refused, because that
study’s `CENS` is meaningful only for the PK endpoint — the PD effect is
signed, so a “left-censored” PD row reports a value above uncensored
ones and the flag cannot mean what it means for PK:
...
Leaving `cens` undeclared is the right answer for this study, and it is
what the run above does.
...
Everything case1_pkpd’s run removed, and what was left to build on.
{.table}

**This is what a clean result looks like, and it is worth seeing once.**
All 180 patients start with a unique observation schedule and **none**
ends with one. The difference from every dataset above is a single
declared column: with `NOMTIME` present the grid is `nominal` rather
than inferred, so coarsening is reading the protocol instead of guessing
at it. 180 patients collapse onto **six** distinct visit sets, none of
them rare, so nothing was discarded and no arrangement had to be
invented. Both output guarantees are 0.

Read this next to `nimoData`, which is the same mechanism with nothing
to work from. The gap between them is not a tuning difference; it is
whether the study recorded a nominal time.

Dose is not recomputed: the five distinct amounts are the arm’s assigned
dose, not a multiple of baseline weight, so detection correctly
declines.

## mad: five endpoints, including ordinal, count, and binary

[`xgxr::mad`](https://rdrr.io/pkg/xgxr/man/mad.html) is a
multiple-ascending-dose study carrying five observation endpoints — PK
concentration, continuous PD, and ordinal, count, and binary PD — keyed
by `NAME`. Everything else here has one or two, and an endpoint silently
...
#> synpmx_avatar(): dropped 6 undeclared column(s): TIMEUNIT, EVENTU, CENS, PROFDAY, PROFTIME, CYCLE.
#>   Declare a column in `keep` to carry it through verbatim.
```

Every endpoint in is an endpoint out. This is a set comparison, not a
row count: row counts stayed plausible in the defect that motivated the
check while an entire compartment disappeared.
...
![](avatar-evaluation-public-data_files/figure-html/mad-plot-1.png)

The ordinal, count, and binary panels are the ones to look at. AVATAR
blends donor values and then restores the source column’s class, so a
count endpoint comes back as counts and an ordinal one keeps its levels
— it does not learn that they are discrete, it never destroys the type
in the first place.
...
|:---|---:|:---|
| **Who was available to build on** |  |  |
| Patients in the source | 60 |  |
|   excluded as structurally extreme | 0 (0%) | `screen`: follow-up or dose count over twice the cohort’s 90th percentile |
|   excluded, route arm too small | 0 (0%) | `on_donor_shortfall`: a route arm holding fewer than k + 1 patients |
|   left to anchor avatars on | 60 (100%) | an excluded patient still contributes as a donor |
| Avatars built | 60 | cohort size is unaffected by the exclusions above |
| **Donor pools: who may be blended with whom** |  |  |
| Administration routes | 1 | oral, infusion, and so on. Donors are NEVER blended across a route, so each is a separate pool |
| Dose/schedule groups | 6 | patients with an identical dose pattern and endpoint set. Donors are looked for here first; many small groups means the search falls back to the wider route pool |
| **How much of one real patient reaches one avatar** |  |  |
...
|   held by fewer than 2 patients, so not reused | 0 (0%) | `min_pattern_share` is that threshold. These visit sets are lost, not approximated |
|   real patients holding those | 0 (0%) | those patients are NOT removed – they still anchor avatars and still act as donors. Only their particular pattern of absences stops being copied |
| Avatars given a visit set from the pool | 60 of 60 (100%) | drawn from the sets that cleared the threshold, or built from their shape – never from their own anchor alone |
|   of those, misses placed fresh | 0 of 60 (0%) | the kind of missingness was reused; exactly which visits were missed was invented |
|   of those, miss count moved | 0 of 60 (0%) | no arrangement at the wanted number of missing visits was free, so the count moved by a visit or two. Misses at the END of a record are the case that forces it, because for a given count there is exactly one such arrangement |
|   of those, a rare set swapped for a shared one | 0 of 60 (0%) | the anchor’s own set was held by nobody else and no arrangement was free, so the group’s most widely held set was used instead – less faithful to that avatar, and it discloses nothing |
|   of those, moved to a different anchor | 0 of 60 (0%) | the first anchor’s own set was shared by nobody and nothing legal could be placed, so this avatar was anchored elsewhere. Every source patient stays a donor and stays available to anchor others |
| Avatars keeping their anchor’s own visit set | 0 of 60 (0%) | not a problem in itself: if several real patients share that set, copying it identifies nobody. Only the next row is a disclosure |
| **Avatars carrying a visit set nobody else shares** | 0 (0%) | **this is the row that must be 0%.** That pattern of which visits have observations belongs to one real patient. It is non-zero only when the schedule group has no shared set to substitute; the run alerts when it happens |
|   of those, dosing re-truncated | 0 of 60 (0%) | the anchor stopped dosing at a depth nobody else used, so the avatar stops at a different one – shared, or used by nobody. Truncating a schedule to a real dose time is protocol-valid in a way that moving dose times is not |
| Distinct dose schedules in the source | 2 |  |
|   represented in the synthetic cohort | 2 (100%) | a regimen only one patient received cannot be given to an avatar without pointing at them, so it is not represented at all. This is the cost of the guarantee below, and on a small cohort it is unavoidable rather than a setting to tune |
| **Avatars carrying a dose schedule nobody else shares** | 0 (0%) | **must also be 0%.** Dose events are copied from the anchor verbatim, so patients whose dose times nobody shares are not built upon. Non-zero only when EVERY patient is in that position, which individualised dosing can cause |
| **Dose** |  |  |
| Amounts recomputed from a covariate | **no** | the 5 distinct dose amounts are not a fixed multiple of any declared covariate: WEIGHTB (39 ratio levels for 5 distinct amounts – too many to be a protocol); SEX (missing or non-positive on dose rows) |
|   so `amt` is copied verbatim | from the anchor | each avatar’s implied dose per kg is therefore its anchor’s, not its own, and the amount still encodes one real patient’s covariate. Declare `dose_covariate` if this study is weight- or BSA-based |

Everything mad’s run removed, and what was left to build on. {.table}

Another declared `NOMTIME`, and the same clean outcome: 60 of 60 unique
before, 0 after, six visit sets, nothing discarded, no arrangement
invented, both guarantees at 0. Five endpoints do not make masking
harder — they make *endpoint loss* detectable, which is why this dataset
is here.
...
is 59 neonates given phenobarbital for seizure prevention, and the data
come from routine clinical care rather than from a protocol. Dosing is
individualised to the infant, sampling is sparse and irregular, and
weight varies over time. It is the least protocol-like dataset here, and
...
![](avatar-evaluation-public-data_files/figure-html/pheno-plot-1.png)
...
**Read the dose rows, not the observation rows.** On the observation
side the mechanism works: the visit-set guarantee holds at 0, so no
avatar wears a set of attended visits that belongs to one real infant.
On the dose side it does not, and the run says so rather than passing
quietly — **15 avatars at this seed carry a dose schedule nobody else in
the cohort shares**, which is the row that is supposed to be 0. In the
source, 55 of the 59 infants have a dose schedule nobody else has, so
there is very little to anchor on safely.

The cause is not a setting. Dose events are copied from the anchor
verbatim, because moving a dose is not a protocol-valid edit the way
moving a sample is. When dosing is individualised — a neonate dosed to
their own weight, on the day their own clinician decided — most patients
have a dose schedule nobody shares, and there is no safe patient to
anchor on instead. Coarsening cannot help: it acts on the visit grid,
and the exposure here is *which days this infant was dosed*.

That is what an honest failure looks like, and it is why every run
reports both guarantees separately. A study with this shape either
accepts that its avatars carry real dosing schedules — defensible only
inside the source data’s own access controls — or does not use AVATAR.

Dose is not recomputed here either, for the same reason as `theo_md`:
the 54 distinct amounts are not a clean multiple of `WT` or `APGR`, so
...
![](avatar-evaluation-public-data_files/figure-html/empirical-plot-1.png)
...
`pattern_generated_fraction`, which the per-dataset tables above print
...
of visits to hand the avatar that would otherwise have worn it. The
per-dataset tables show the price in the “misses placed fresh” row, and
it varies as much as everything else here: `nimoData` pays it in full at
100%, `pheno_sd` at 41%, `warfarin` at 22%, `wbcSim` at 2%, and
`mavoglurant`, `theo_md`, `case1_pkpd` and `mad` not at all.
