...
|  | Supported |
|----|----|
| PK | `1cmt_iv`, `1cmt_oral`, `1cmt_infusion`, `2cmt_iv`, `2cmt_oral` — closed form, no ODEs. `1cmt_mixed` and `2cmt_mixed` exist for a study dosed by two routes, but only [`synpmx_model()`](https://iamstein.github.io/synpmx/reference/synpmx_model.md) can use them: it reads each dose’s route from the study, and a public design cannot state one |
