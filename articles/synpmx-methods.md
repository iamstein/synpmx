...
and it is where the differences between these modes are most visible.
...
prior_only <- synpmx_prior(theo_model, theo_design, public_roles,
                           n_subjects = 12, seed = 202)
head(prior_only, 3)
#>   ID      TIME NTIME       DV AMT EVID CMT
#> 1  1 0.0000000  0.00       NA 320    1   1
#> 2  1 0.0000000  0.00 0.000000   0    0   2
#> 3  1 0.2447759  0.25 3.462932   0    0   2
```

Every generator here takes a
[`pmx_roles()`](https://iamstein.github.io/synpmx/reference/pmx_roles.md)
and writes its output under those column names, so one declaration
serves the generator,
...
The three that read a study take it as a description of what they are
reading. This one reads nothing, so the same object does the opposite
job: it says which schema to generate into, and that is what makes the
table above comparable to `theo_md` without renaming a column.
[`pmx_generated_roles()`](https://iamstein.github.io/synpmx/reference/pmx_generated_roles.md)
is the default, and it names every column the generator can produce —
nominal time, time after dose and occasion included.

The clearance of 6 L/h assumed above is about twice the truth for
...
calibrated_data <- synpmx_calibrated(
  data = theo_md, roles = public_roles, model = theo_model,
...
calibrated <- synpmx_calibrated(
  data = confidential, roles = public_roles, model = theo_model,
...
the comparison above is worth being able to check. It is not under
active development at present and gets no documents of its own. Reach
for it only on a large pooled dataset, and read [the privacy
article](https://iamstein.github.io/synpmx/articles/synpmx-privacy.html)
first.
...
# the two generating under `public_roles`, which declares no covariate -- simply
# has no rows here.
...
|  | Supported |
|----|----|
| PK | `1cmt_iv`, `1cmt_oral`, `1cmt_infusion`, `2cmt_iv`, `2cmt_oral` — closed form, no ODEs, plus `1cmt_mixed` and `2cmt_mixed` for a study dosed by more than one route, which need `pmx_trial_design(routes = )` to say which dose went which way |
...
enumerates the catalogue from the code itself and demonstrates the
exposure-independence of the PD forms on two dose levels, which is the
limitation in this list most likely to be mistaken for a result.
