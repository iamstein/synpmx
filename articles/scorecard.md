...
helps assess output from any generator. Checks that need a particular
mechanism or run record report `not applicable` when it is absent. The
worked examples here use
...
A5a and A5b are events per patient, split by type: observations in one,
doses in the other. **Doses, not dose records.** A regimen written as
one row plus `ADDL` and `II` stands for as many doses as `ADDL` says, so
both tables are expanded before anything here is counted; otherwise the
row would score the encoding rather than the study. On `onc_sim` — 322
dose records standing for 71,180 doses — the difference is the
difference between reading 1.18 against 7.48 and reading 327 against
286. Each passes when the synthetic count lands within 5% of the
source’s — that much movement leaves each patient carrying the same
amount of information, and there is nothing to decide. Further than that
is `review` and never `FAIL`, because the number is permitted to move: a
shortened dose course can be the correct answer, as the rest of this
section shows.
...
![](scorecard_files/figure-html/a-schedule-1.png)
...
![](scorecard_files/figure-html/a-schedule-2.png)
...
**B2 reads `not applicable` for a generator that simulates its
profiles**, the same shape of exemption B4a carries and decided from the
same `"pmx_source"` attribute. The row exists because an avatar is built
from a real patient’s trajectory: an avatar that stands out stands out
the way its anchor did, and singling it out is singling somebody out.
...
draw a patient’s whole record from a model of the cohort, so an outlier
there is the tail of a distribution the generator was given — a utility
reading rather than a disclosure one, and scoring it in section B
invites it to be read as a leak it is not.
[`flag_identifiable_subjects()`](https://iamstein.github.io/synpmx/reference/flag_identifiable_subjects.md)
still runs on any table for a reader who wants that reading.
...
![](scorecard_files/figure-html/d-figure-1.png)
...
### E1, E2. Did the model that made the data actually fit

The rows above all score the synthetic table. These two score the
*model* that produced it, and they exist because a population fit can
fail in a way no property of the output reveals.

Where the optimizer takes no effective step, `nlmixr2` still returns an
objective, an AIC and a full parameter table — of starting values. The
generator simulates from those values, and every row above passes,
because they ask whether the output copies anybody or changed the
study’s shape and a fit that never moved does neither. That is
`SIM-075`, found on a real study whose report read `f 0.7` — the
starting value exactly — beside three between-subject terms all reading
0.316, which is `sqrt(0.1)`, the eta initial value.

- **E1** passes when every fitted concentration model moved off its
  starting values, judged at a deliberately loose 1% per parameter: the
  question is whether the optimizer moved at all, not whether it
  converged tightly. A model that did not move is named.
- **E2** passes when the between-subject terms were estimated rather
  than left at their starting value while the fixed effects moved. That
  is the narrower failure, and the stored `case1_pkpd` fit was it for
  months: clearance, volume and absorption estimated, all three omegas
  sitting on 0.1, and a synthetic spread 0.68 of the source’s as the
  visible consequence.

Both are `review` and never `FAIL`. The numbers may still serve the
