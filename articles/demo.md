...
why: variance is shrunk by blending.

The masking report’s dose row is not a second reason here. `case1_pkpd`
gives every patient in an arm the same milligram dose, so M5 found no
covariate the amounts are proportional to and left them alone — which is
right, and discloses nothing, because thirty patients share each amount.
On a weight- or body-surface-area-based study that row matters: declare
`dose_covariate` in
[`pmx_roles()`](https://iamstein.github.io/synpmx/reference/pmx_roles.md),
or each avatar carries its anchor’s milligrams and therefore its
anchor’s weight.
