...
The literature on **checking** synthetic data is separate from the
literature on generating it, has its own vocabulary, and is easy to
reinvent badly if you have not read it. It is spread across three
disciplines: statistics, machine learning, and data protection law. This
article is written as a tutorial rather than a survey, and it says at
each step which measures `synpmx` implements, which it does not, and
why.

Its companion, [the generation
review](https://iamstein.github.io/synpmx/articles/synthetic-data-generation-review.html),
surveys the four families of generation method in use and says where
`synpmx` fits among them. Current as of August 2026.

## Checking Synthetic Data

### Why this is hard: population facts versus patient facts

A synthetic dataset is *supposed* to tell you about the population. That
is what it is for. It must not tell you about a patient and each privacy
check in this is an attempt to separate those two.
...
measure in this article except differential privacy.
