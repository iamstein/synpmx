...
#### Choosing the fraction from the effect you would act on

The size of the control set decides the smallest privacy loss the
procedure can see. Making that concrete takes three steps, and the
machinery for step 2 already exists as the null interval in
[`compare_pmx_proximity()`](https://iamstein.github.io/synpmx/reference/compare_pmx_proximity.md).

1.  Name the smallest privacy loss you would act on, $`\delta`$, on the
    adversarial accuracy scale.
2.  For a candidate control size $`m`$, compute the null: split the real
    cohort into two disjoint sets of size $`m`$, compute $`\mathrm{AA}`$
    between them, and repeat. The spread of that null is the noise floor
    at size $`m`$.
3.  Take the smallest $`m`$ whose noise floor is comfortably inside
    $`\delta`$.

The output is a statement of the form: *at this cohort size, a control
set smaller than $`m`$ cannot detect a privacy loss of $`\delta`$.* The
missing input is $`\delta`$ itself, for which no published value exists.
Until one is chosen, the useful version of this calculation is its
negative result, which is available today: compute the null at the size
your cohort allows and read off the smallest difference the procedure
could ever resolve.
