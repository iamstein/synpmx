...
``` math
\mathrm{AA}(T,S)=\frac12\left[\frac1n\sum_i \mathbf{1}\{d_{TS}(i)>d_{TT}(i)\}
...
exists, and calibrates it against a null interval built by recomputing
$`\mathrm{AA}`$ between two random halves of the source cohort. At 20
patients in a 30-dimensional profile space, nearest-neighbour statistics
are unstable and 0.5 is *not* the right expectation, so a raw
adversarial accuracy is uninterpretable without that null. The null
interval states what this statistic does on data of this size and shape
when nothing is wrong.

That null costs half the source cohort. Both halves of the null and both
arms of the comparison run at one size,
$`n = \min(n_{\text{source}}/2,\,
n_{\text{synthetic}})`$, so a 24-patient study is compared 12 against
12.
...
1.  Split the source cohort into a training set and a control set,
    **stratified by arm**, so that no arm can be lost by an unlucky
    draw.
2.  Generate from the training set only.
3.  Compute $`\mathrm{AA}(T,S)`$ and $`\mathrm{AA}(C,S)`$ at one size,
    subsampling the larger real set down to the smaller, and take the
    difference.
4.  Repeat steps 1 to 3 many times with different splits, and report the
    distribution of the difference rather than one value.
5.  Report the split fraction alongside the numbers, because the
    estimate depends on it.

#### Two floors on the training set

The split must leave the generator able to run, and two different
mechanisms set two different floors.

- **At least `k` + 1 = 6 subjects per route.** Donors are never blended
  across routes, so a route holding fewer than 6 subjects has no legal
  donor set and `on_donor_shortfall = "drop"` removes those subjects
  from the anchor pool entirely. A route here is the combination of
  dosing compartment, event type, and bolus against infusion, so a
  single-route study gives every subject the same key and this floor is
  satisfied automatically. It binds only on a study mixing routes, such
  as an intravenous lead-in followed by oral dosing.
- **At least 3 subjects per stratum.** Donors *do* cross strata to reach
  `k`, so a stratum of 2 does not stop generation. This floor is about
  fidelity: strata holding fewer than three source patients are
  deliberately not size-balanced, because reproducing a one-patient
  cell’s size exactly would disclose it. Below 3, that arm’s synthetic
  size varies with the seed. Relaxing the floor to 2 is a defensible
  choice with that cost attached.

#### Why repeat the split rather than enlarge it

Split size trades two errors against each other, the same trade as in
cross-validation.

- A **large control set**, such as a 50/50 split, estimates
  $`\mathrm{AA}(C,S)`$ precisely. It also trains the generator on half
  the cohort, so the number describes a generator that is not the one
  you will ship.
- A **small control set**, such as 20%, characterizes very nearly the
  generator you will ship, and estimates its privacy loss noisily.

Repetition removes the noise. Repetition does **not** remove the bias
from training on fewer patients, so the two are not interchangeable, and
the reason to prefer a small repeated split is that its bias is the
smaller of the two problems.

One caveat specific to this statistic. Adversarial accuracy depends on
how many candidates each nearest-neighbour search had, so both arms must
be computed at one size, and that size is set by the *smaller* set. A
20% control set on 24 patients means both arms are computed at 5
subjects. That pushes toward a larger fraction than cross-validation
practice would suggest, and where the balance lies has not been
measured. **The direction of the small-training bias is also
unmeasured**: fewer donors makes each avatar a blend of less similar
patients, which could move the estimate either way. Measuring it means
running this procedure at several fractions on a public dataset and
watching the estimate move.

#### What a privacy loss of 0.1 means

$`\delta = \mathrm{AA}(C,S)-\mathrm{AA}(T,S)`$ is a difference of two
accuracies, so it inherits their units, which are none. It converts into
a count of patients by the following argument.

Suppose a fraction $`p`$ of the training subjects have a synthetic
near-copy. For each of them, the nearest synthetic subject sits closer
than the nearest other real subject, so their indicator is 0 rather than
the $`1/2`$ it would be if the synthetic data were an independent draw
from the same population. The same holds on the synthetic side for the
copies. Averaging gives

``` math
\mathrm{AA}(T,S)\approx\tfrac12(1-p),
\qquad
\mathrm{AA}(C,S)\approx\tfrac12,
\qquad\text{so}\qquad
\delta\approx\tfrac{p}{2}.
```

**A privacy loss of $`\delta`$ means roughly $`2\delta`$ of the compared
set has a synthetic near-copy.** The compared set, not the source
cohort: both arms run at one size $`m`$, and
[`compare_pmx_proximity()`](https://iamstein.github.io/synpmx/reference/compare_pmx_proximity.md)
sets $`m`$ to half the source cohort so that the null has two disjoint
real halves to work with.

| $`\delta`$ | Fraction memorized | At $`m`$ = 12 | At $`m`$ = 500 |
|----|----|----|----|
| 0.01 | 2% | 0.2 patients, below the resolution of the cohort | 10 patients |
| 0.10 | 20% | 2 patients | 100 patients |
| 0.50 | 100% | every patient | every patient |
| 1.00 | not attainable | — | — |

$`\delta = 0.5`$ is the ceiling, reached when the synthetic data is a
copy of the training set. $`\delta = 1`$ would require the synthetic
data to be simultaneously a copy of the training set and perfectly
separated from the control set, which means the split was not random.
Treat a value near 1 as a broken experiment rather than as a privacy
state.

The floor is set by cohort size, because $`p`$ is a fraction of patients
and the smallest non-zero fraction is one patient:
$`\delta_{\min}\approx 1/(2m)`$. One memorized patient is worth
$`\delta = 0.042`$ on a 24-patient study, where $`m = 12`$, and
$`\delta = 0.001`$ on a 1000-patient one, where $`m = 500`$.

#### The noise floor, measured on public data

The null interval in
[`compare_pmx_proximity()`](https://iamstein.github.io/synpmx/reference/compare_pmx_proximity.md)
is the noise floor: what $`\mathrm{AA}`$ does between two halves of the
...
**The noise floor falls as $`1/\sqrt{n}`$ while one patient’s
contribution falls as $`1/n`$.** The gap between them therefore *widens*
