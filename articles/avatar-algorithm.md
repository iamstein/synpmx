...
- **(d) Build each synthetic subject** — *Steps 6, 7, 8, and 9*. Copy a
  ...
  Recompute the dose from the avatar’s own blended covariate when in the
  original dataset dosing is proportional to a covariate, as it is for
  weight-based dosing (**M5**). Then reconstruct any below-limit (BLOQ)
