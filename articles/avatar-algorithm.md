...
Both are read from the doses as given. A regimen recorded as one record
plus `ADDL` and `II` is written out by
[`pmx_expand_doses()`](https://iamstein.github.io/synpmx/reference/pmx_expand_doses.md)
before this step, so a year of daily dosing is a year of dose rows here
and not one; the generated study is folded back by
[`pmx_compress_doses()`](https://iamstein.github.io/synpmx/reference/pmx_expand_doses.md)
on the way out, so it leaves in the encoding its source used
(`REV-051`).
...
``` math
\bigl(\mathrm{ADM},\ \mathrm{EVID},\ \mathrm{CMT},\ [\mathrm{RATE}\neq 0]\bigr)
```

tuples on its dosing rows, so the compartment a dose enters and whether
...
The first term is the route the study *declared*, through
`pmx_roles(adm =, routes =)`. The other three are proxies for a route a
study did not state, and in exactly the studies the declaration exists
for they are blind to it: a Monolix-style table routes by `ADM` alone,
so an intravenous bolus and a subcutaneous dose carry the same EVID, the
same compartment and the same zero rate. Without the declared term those
two patients key alike and blend into each other, across the one
boundary this step exists to hold. An administration id that `routes`
does not cover keys on its own raw value, so two uncovered ids stay
apart rather than collapsing together.
