...
- `strata` are *assigned* strata — arm, dose group, cohort. They are
  ...
  and the pool of visit patterns an avatar may be given. In cases where
  you have two columns that declare a single strata and you want to keep
  both (e.g. a string (`TRT`) and numeric (`TRTN`) column, then placae
  one of them in `keep`.)
- `keep` is the escape hatch for anything else you want carried through
  ...
  value, so keep only what the source data’s own access controls already
  permit.
...
| [Checks of the synthetic data](https://iamstein.github.io/synpmx/articles/scorecard-synthetic-data-checks.html) | I have a synthetic dataset. Should I use it? |
