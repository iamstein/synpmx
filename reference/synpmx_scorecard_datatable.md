...
`"unavailable"` and `"not applicable"` in muted grey, and `"pass"` left
as ordinary text. A card is five verdicts among thirty-odd rows of
prose, and the rows that need reading are the ones that have to be
findable without reading all of it.
