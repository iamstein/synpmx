...
- `unmaskable_dosing` – their set of dose times is shared by fewer than
  ...
  schedule that could be moved to a shared depth. Dose events are copied
  from the anchor as they stand, since resampling them would emit
  regimens the protocol never permitted, so there is nothing to
  substitute.
