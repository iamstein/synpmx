...
`theo_md` is a four-column dataset. A real study is not, so here is the full
declaration. The roles are also the **manifest of what survives**:
`synpmx_avatar()` drops every column no role names, so a column you forget is
dropped rather than quietly copied out of a real patient.

Only `id`, `time`, `dv`, and `evid` are required. Everything else is optional,
and this example declares all of it at once:
...
- `subject_properties` are *assigned* strata — arm, dose group, cohort. They are
