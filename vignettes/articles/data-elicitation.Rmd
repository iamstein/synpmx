...
`pmx_covariates_reference()` supplies one for the usual covariates and
`pmx_covariate_reference_table()` lists what it holds with its units. Those are
package reference values for a broad adult population, not a rendering of any
trial, and the spread is the half worth reusing: body weight holds much the
same coefficient of variation across adult cohorts while the median moves, so
`medians` adjusts the one that does not transfer. Age, sex and race are
protocol facts that no reference population can supply — answer questions 17
and 18 instead.
