---
title: "Describing the trial: a data-structure elicitation guide"
---
...
The companion to [the model elicitation guide](model-elicitation.html). That guide asks **what the drug
...
in the package specification, the second is the more serious failure:
...
- **Generator needs:** the dose levels, cohort sizes, and the dose-group
  assignment as a **stratum** (see `pmx_roles(strata=)`).
...
- **Generator needs:** the dosing interval, number of doses, and **which
  ...
  was a defect. A Q24H regimen does not mean Q24H sampling.
...
Throughout the package specification the protocol is treated as a public input
...
where a reviewer can see it. It is not a leak to remediate.
...
3. **Does any subject appear more than once** across the studies you intend to
   ...
   and must be answered before pooling.
...
17. **What are the inclusion criteria for each one** — the age window, the
    weight or body mass index limits, the renal cutoff. These are the public
    bounds, and they come from the protocol rather than the data.
18. **What did enrollment target** — the sex split, the countries and sites.
    A trial that enrolled in one region has a race distribution that has
    nothing to do with any general population.
19. **Which unit is recorded** — height in centimetres or metres, creatinine
    clearance in mL/min or mL/s. This is the question that most often makes a
    generated column wrong by a factor rather than by a little.

### Declaring a covariate without reading the data

The bound and the distribution are separate answers. `range` is the public
limit a value is clipped to, so it should be generous; `median` and `cv` say
where the population sits inside it.
...
Supplying a range and nothing else leaves the generator with only the bound to
work from, and it centres a symmetric draw on the midpoint six standard
deviations wide. That is rarely what a covariate looks like, and widening the
bound for safety then widens the distribution with it.

Where a distribution has to be assumed rather than elicited,
`pmx_covariates_reference()` supplies one for the usual covariates.
`pmx_covariate_reference_table()` lists what it holds, in which units, and on
what basis: body size from NHANES adults, the laboratory values from
conventional reference intervals, and age, sex and race marked as placeholders
because questions 17 and 18 are what decide those.

A survey is not a trial cohort, so treat those as a starting point rather than
an answer. NHANES describes the general adult population and eligibility
criteria cut its tails, so a trial usually runs both lighter and tighter — a
weight median around a tenth below the survey's, at a coefficient of variation
nearer 15% than 25%. `medians` and `cvs` move each, and a covariate whose
median you replace records that in its own `source` rather than going on citing
the survey:
...
A cohort selected for obesity, or for renal impairment, runs the other way.
...
[the model elicitation guide](model-elicitation.html):
...
Extend the table in [the model elicitation guide](model-elicitation.html) with the design rows:
