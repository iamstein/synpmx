---
title: "Feasibility: what can be released, and from how many patients"
---
...
Companion to [the mechanism-level privacy argument](privacy-argument.html).
This article asks a scoping question rather than an implementation question:
...
after the presence-decoding fix. Error is the median absolute deviation of the decoded
...
Viable at N in the high hundreds given a tighter sensitivity bound, a zCDP
accountant with a Gaussian measurement, and a low-dimensional trajectory
release; needs thousands at epsilon 1 without them. Appropriate for **pooled** corpora: many
...
is *within* the source data's own access controls, where the alternative is
passing the real data around — but that is a different product from the one this
...
2. **Scope the DP path explicitly to pooled data.** State a minimum viable
   ...
   500 at epsilon 5, or N >= 2,500 at epsilon 1. Run the `pmx_preflight()`
...
- **A tighter sensitivity analysis.** If the true per-subject L1 is
...
the package specification: "public" in the DP sense means
...
mechanism improvement**, and tightening priors is cheaper than any of the three
improvements listed for Tier C above.
...
Gaussian noise under zCDP is recommended for Tier C above. That advice is
**dimension dependent and does not apply to Tier B**. At `d = 6` and `epsilon = 1`, Laplace
...
4. **Linear PK is an assumption, not a fact.** Dose-normalization is invalid
   ...
   design in the package specification, where the model states its
...
2.8-fold. Against the accuracy bar in the package specification that
