---
title: "Building the structural model: an elicitation guide"
---
...
Companion to [the data elicitation guide](data-elicitation.html), which asks
what the dataset looks like rather than what the drug does. If the `d` and `f`
arithmetic in Q7 is unfamiliar, read
[the privacy background article](privacy-background.html) first.
...
arithmetic ([the feasibility article](feasibility.html) section 8). A range set by peeking at the
...
[The synpmx data generation algorithms](synpmx-methods.html) is the full
selection guide, including how many parameters each mode learns from the data.
