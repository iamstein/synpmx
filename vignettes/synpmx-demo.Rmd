...
schedules are unique, so a subject can be alone in its exact event-pattern
group. `synpmx_avatar()` still blends each synthetic subject from `k` real
patients (default 5) by borrowing the nearest donors from other schedule groups
when a subject's own group is too small — the avatar keeps its anchor's event
skeleton while its measurements are averaged across several patients, so no one
source subject is reproduced near-verbatim.
