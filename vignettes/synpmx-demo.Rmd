...
- **AVATAR blending** (`synthesize_pmx()`) — build each synthetic subject from
  real subjects. No elicitation, no formal privacy guarantee.
- **Prior only** (`pmx_generate()` on a public structural model) — read no data
  at all.
- **Calibration** (`fit_calibrated_pmx()`) — simulate from a public structural
  model whose magnitude is corrected by a small differentially private release.
- **Empirical** (`fit_private_pmx()`) — release a dense set of differentially
...
2. synthesize with `synthesize_pmx()`;
...
anchor, `synthesize_pmx()` uses that anchor as the sole donor and randomized
...
The fit reads the data once; generation is post-processing, so any number of
datasets can be drawn from one release without spending more budget.
...
fails closed when it is unavailable. The `fit_calibrated_pmx()` mode, which
