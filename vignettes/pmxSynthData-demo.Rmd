...
`pmxSynthData` creates **synthetic pharmacometric data for model-workflow
...
pmxSynthData" vignette applies all four to one dataset side by side:
...
of the differentially private modes instead (see the privacy-introduction
vignette); the tradeoff is discussed in `design/METHOD_DISCUSSION.md`.
...
environment, not for release across a trust boundary. See the method vignette
for how the blending works, the introduction vignette for all four modes side by
side, and the privacy-introduction vignette for what differential privacy does
