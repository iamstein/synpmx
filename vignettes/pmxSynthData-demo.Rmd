...
`pmxSynthData` creates **mock pharmacometric data for model-workflow
...
This vignette uses the default method, **AVATAR-style blending**
(`synthesize_pmx()`). For each generated subject it samples a compatible source
subject's event skeleton as a template, then fills the covariates and endpoint
trajectories with a distance-weighted blend of similar subjects, plus subject
and residual noise. The output has the source schema, fresh identifiers, and the
same cohort size.

The output is **not** anonymous and carries **no formal privacy guarantee**. It
is mock data for a trusted computing environment, in the same spirit as
...
clinical decisions. When the generated data must cross a trust boundary, use the
differentially private engines instead (see the privacy-introduction vignette);
the tradeoff is discussed in `design/METHOD_DISCUSSION.md`.
...
knitr::kable(
  source_mock_preview(theo_md, theo_synth),
...
knitr::kable(
  source_mock_preview(warfarin, warfarin_synth),
...
for how the blending works, and the privacy-introduction vignette for the
