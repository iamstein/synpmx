...
| `REV-024` | AVATAR donor selection | `.select_donors()` (`R/synthesis.R:363-398`) requires an anchor and a candidate donor to have an exactly equal `.event_signature()` string (`R/profiles.R:1-52`) before any distance-based neighbor ranking runs. The schedule component of that signature encodes the exact dose count and the exact rounded inter-dose gap sequence, so for daily-dosing regimens with per-patient interruption timing or variable follow-up duration (dose-hold-heavy phase 3 oncology; variable-duration phase 1 dose-escalation), the signature is close to unique per patient — two patients on an identical 40-day regimen differing only by one 3-day toxicity hold produce different signature strings (worked arithmetic in the new design doc) and are therefore never candidate donors for each other. Most anchors end up in a signature group of size one, so `.synthesize_covariates()`/`.synthesize_trajectories()` "blend" the anchor with itself — real blending stops happening precisely for the richest, most individually-shaped event tables, which is also where a copied trajectory is most fingerprint-like (the asymmetry already recorded in `design/PROTOTYPE_SPEC.md:1020-1027`). The degeneracy itself was already known and written up twice as a privacy argument (`design/PROTOTYPE_SPEC.md:1075-1086`, `vignettes/articles/feasibility.Rmd:73-79`), but not as a blending-quality problem for AVATAR's stated trusted-environment use, and no fix was ever proposed. | Full scoping in `design/AVATAR_DONOR_SIMILARITY.md`. **Decided 2026-07-27:** drop the hard signature-equality gate in `.select_donors()` completely — no coarse gate either. Route/dose-sign/`CMT`/`DVID`/endpoint set and a schedule-shape feature vector (dose count, on-drug fraction, interruption count, max/median gap) all become numeric features folded into the existing PCA distance, so the *k* nearest donors are chosen continuously with no partition step at all; separately, reconsider `.interpolate_trajectory()`'s linear interpolation for target times near a donor's own drug-holiday, favoring bounded-interpolation-with-nearest-time-fallback so a smoothed value is never invented across a gap the donor's own record doesn't support. Not implemented; the design doc's open-questions list is now centered on making the former hard-gate signals (route/dose/endpoint mismatch) dominate distance rather than get diluted, plus feature list, `k` semantics once there is no partition, which time-borrowing option to try first, and missing test fixtures (including a mixed-regimen fixture to confirm the removed gate doesn't cause cross-regimen blending in practice). | open |
