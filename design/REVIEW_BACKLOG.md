...
| `REV-031` | Inferred grid, merge vs snap | `.derive_time_grid()` merges pooled times into visit cells under an explicit constraint — a cell must never hold one subject twice, since collapsing two of a subject's own visits destroys a real visit rather than hiding a schedule — and the comments state that guarantee. But `.snap_to_grid()` then assigns each time to the **nearest cell centre**, which is a separate decision that does not know about cell membership. A time near a boundary can land in a neighbouring cell and break the constraint the merge just enforced. On `nimoData`, 35 of 321 observations end up sharing a time with another observation of the same subject; example, subject 1's 1512.11 and 1512.90 both snap to 1513.09. Found 2026-07-30 while writing `scripts/explore_nimodata_coarsening.qmd`. | **Open, low priority.** The magnitude is small next to what a deliberately coarser grid costs (a one-week grid on the same data causes 164 such collisions), and no conclusion in the exploration script changes. It is also now harmless rather than lossy: since `SIM-036`, `.attendance_key()` numbers repeated cells so a collision no longer deletes a row. The fix is to assign each time to the cell it was merged into rather than to the nearest centre, which means carrying the membership out of the merge instead of recomputing from the centres. Worth doing when the grid is next touched; not worth a standalone change. | open |
