...
| `REV-017` | Public inputs | The protocol is treated throughout as a public input costing no budget, but in escalation and adaptive designs the *realized* design is a function of the data. Stopping at 100 mg implies a dose-limiting toxicity at 300 mg; a 3+3 cohort of six rather than three implies a DLT occurred. These are statements about a handful of individuals released with no accounting. Prespecified design is public; realized design that depended on the data is not. | Require an explicit public-source assertion for realized design quantities, recorded in the ledger; or generate from the planned design instead, which discloses nothing and is adequate for workflow testing; or budget the realized counts. See `design/DATA_ELICITATION.md` section 3. | open |
