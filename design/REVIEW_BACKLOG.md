...
| `REV-023` | API / Governance | The DP engines' unaudited status (`README.Rmd` "Maintenance status": complete and tested, not under active development, known open findings, not independently privacy-audited) is documented but not enforced. `synpmx_calibrated()` and `synpmx_empirical()` are exported functions exactly as frictionless to call as `synpmx_avatar()` — nothing at the call site distinguishes "principled demonstration of the privacy/utility tradeoff" from "production release mechanism." The durable contribution of the DP path is the reasoning that ships alongside it — the cohort-size feasibility analysis (`REV-002`, Part 3 above, `vignettes/articles/feasibility.Rmd`) and the trust-boundary decision rule (`README.Rmd`, `synpmx-privacy.Rmd`) — not the specific mechanism implementation, which still carries open P0/P1 findings (`REV-003` unaccounted `stop()` channel, `REV-004` decorative `delta`). Risk shows up at the moment someone decides to ship the output, and prose alone does not stop that moment from arriving silently in a script or pipeline. | **Fixed.** Added exported `synpmx_enable_dp_engines()` / `synpmx_disable_dp_engines()` (`R/synpmx.R`), which set an in-session-only flag (not persisted across restarts). `synpmx_calibrated()` and `synpmx_empirical()` check it first thing and `stop()` with the audit-status text and a pointer to `vignette("synpmx-privacy")` if it was never called; `backend = "public"` is exempt, since it makes no DP claim. `synpmx_enable_dp_engines()` also `message()`s the acknowledgment, so it is visible in a transcript without reading the source. Regression tests in `tests/testthat/test-synpmx-api.R` (unacknowledged calls error, `backend = "public"` is exempt, enabling unlocks the real backend). Callers updated: `scripts/demo_nlmixr2data.R`, `scripts_private/try_dp_calibrated.R`, `vignettes/synpmx-method.Rmd`, `vignettes/synpmx-demo.Rmd`, `vignettes/synpmx-privacy.Rmd`, `README.Rmd`/`README.md`. | fixed |
| `REV-024` | Documentation / Governance framing | The trust-boundary decision rule is written with a **geographic** boundary when the privacy argument underneath it turns on an **organizational** one. Shipped text says AVATAR output "belongs inside the same trusted environment the source data came from" (`README.Rmd:65`, `synpmx-method.Rmd:115`) and that anything leaving the "safe computing environment" needs a DP engine (`synpmx-privacy.Rmd:140-148`). Read literally, moving AVATAR output from a validated GxP environment onto an analyst workstation is "crossing a trust boundary" and therefore requires DP. **That is the opposite of the intended use case.** The package exists so that synthetic data can leave the validated environment for local model-code development, which is not possible with real data and is the approval the package owner is seeking (raised 2026-07-23). `design/PROTOTYPE_SPEC.md:1014` compounds it by calling trusted-environment use "this package's stated purpose." The privacy argument itself does not depend on which machine the data sits on — it depends on who can reach it and under what obligations — so the fix is a reframe, not a weakening: a laptop under the same corporate access controls and confidentiality obligations is categorically different from a partner, a vendor, or a publication. | **Open.** Reframe the boundary as organizational, not geographic. Canonical rule, to be stated once in `synpmx-privacy.Rmd` and echoed everywhere else: *Does the generated data reach anyone the source data could not?* No — same organization, same access controls and obligations, **including analyst workstations under sponsor policy** → AVATAR. Yes — partner, vendor, publication, or a less-controlled system → a DP engine. What must **not** change: AVATAR output is assembled from real trajectories, is not anonymous, carries no formal guarantee, and inherits the source data's handling obligations wherever it goes. Documentation states the rule; only the sponsor can determine that a given workstation is inside the boundary. **Sites to reframe:** `README.Rmd:65,95` (then re-render `README.md`), `synpmx-method.Rmd:115`, `synpmx-demo.Rmd:234,614-615`, `synpmx-privacy.Rmd:140-148,236`, `articles/avatar-algorithm.Rmd:84`, `articles/feasibility.Rmd:234-236` (light touch; "external release" is already the right axis), roxygen in `R/roles.R:39`, `R/covariates.R:81-82`, `R/synpmx-package.R:4` (regenerate `man/`), and the internal rationale in `design/METHOD_DISCUSSION.md:7,10,124-130` and `design/PROTOTYPE_SPEC.md:34,599,1014-1030`, which will otherwise re-seed the old framing. Already organizational and correct as written: `README.Rmd:79-83`, `synpmx-method.Rmd:365`. **Do not change** — these say "restricted environment" about the internals of the DP fit, a different concept that is still true: `articles/privacy-argument.Rmd:52,177` (source reads and side channels inside the restricted fit) and `R/compare.R:76` (`"restricted_not_releasable"` comparisons that read `source`). **Judgment call:** `R/calibrate.R:378` warns that bootstrap-resampled covariates are not DP while the PK/PD release is. Reword, but consider keeping the restriction *tighter* than plain AVATAR — a release labelled DP invites a reader to treat it as releasable, and these columns are not. **Done 2026-07-23.** Reframed across shipped docs (`README.Rmd`/`README.md`, `synpmx-privacy.Rmd` decision rule, `synpmx-method.Rmd`, `synpmx-demo.Rmd`, `articles/avatar-algorithm.Rmd`, `articles/feasibility.Rmd`), roxygen (`R/roles.R`, `R/covariates.R`, `R/synpmx-package.R`, `R/calibrate.R`; `man/` regenerated), the internal record (`design/METHOD_DISCUSSION.md`, `design/PROTOTYPE_SPEC.md`), and the private templates (`scripts_private/`). `R/calibrate.R` kept the tighter restriction: a DP-labelled release carrying non-DP bootstrap covariates is explicitly not releasable on the strength of that label. Left unchanged as correct: `articles/privacy-argument.Rmd:52,177` and `R/compare.R:76` (internals of the restricted DP fit), and the `scripts_private/` lines about where those scripts run and about not exporting source-derived figures of real data. `./build.sh` Status: OK. | fixed |
| `REV-025` | AVATAR privacy / small donor groups | AVATAR blends only within an exact event-signature group (`R/profiles.R`), and `.select_donors()` (`R/synthesis.R:363`) degrades silently when a group is tiny: a **singleton** group uses its one subject as the sole donor with only subject/residual noise (defaults 0.15/0.05) as perturbation — a noised near-copy of one real patient — and a **pair** group blends exactly two real subjects (`k` collapses to 1). Because the signature includes dose/rate magnitude, a sparse dose arm (typically the extreme dose, or an off-protocol subject) is its own group, so the individuals most identifiable are the ones least blended. The behavior is only a `warning()`; `compare_pmx_distributions()` cannot catch it because the risk is per-subject, not distributional. The demo vignette documents the sole-donor case for `wbcSim` but frames it as expected, not a defect. Found 2026-07-25 by the package owner, who considers silent near-verbatim reproduction unacceptable. | **Open. Design and owner decisions in `design/METHOD_DISCUSSION.md` §6a.** Decisions (2026-07-25): floor = 5, a **hard `stop()`**, not a warning (the loud fix — an error survives `suppressWarnings`, which the warning did not; the demo suppressed it); **no dose-rescaling** — blend nearest-dose donors raw, structure over statistics; **dropping** unique subjects that cannot reach the floor is acceptable. Phase 1 (now): `stop()` below the floor + the post-generation outlier detector (§6a C) as the report surface. Phase 2: relax exact compatibility to a signature distance, expand across nearest dose groups, drop-or-error when 5 is unreachable. Note the paper (`references/Guillaudeux23.pdf`) uses k = 20 headline and k = 4 minimum, so 5 is the feasible floor at PMX cohort sizes, not a privacy recommendation; `max_donor_weight = 0.80` still lets one donor dominate. **Partly fixed 2026-07-25 (commit pending).** `.select_donors()` now prefers same-signature donors then borrows the nearest subjects from other dose/schedule groups to reach `k` (default 5); their trajectories map onto the anchor's times by the existing interpolation, so no source subject is reproduced near-verbatim unless the whole source has fewer than `k + 1` subjects, which now raises a loud red immediate alert (`.loud_warn()`, via `message()` so `suppressWarnings` cannot hide it) rather than a silent warning. The owner chose a loud warning over a hard `stop()` for now. **Key discovery:** because the signature carries dose to 8 sig figs, weight-based dosing (e.g. `theo_md`: 11 distinct doses across 12 subjects) made nearly *every* subject its own singleton group, so the old code was emitting near-verbatim copies of essentially the whole cohort, not just sparse arms. The outlier detector (§6a C) is also done: `flag_identifiable_subjects()` screens follow-up time, dose count, dose magnitude, and DV per subject (`test-flag-identifiable.R`). **Phase 2 done 2026-07-27 (design in `METHOD_DISCUSSION.md` §6a A2).** Route of administration is now an absolute barrier (`.route_key()`): IV/bolus/oral are never blended, at any stage, for any shortfall -- a dose or schedule difference makes a donor a worse match, a route difference makes it a different experiment. Donor selection is explicitly two-stage (exact signature nearest-first, then nearest route-compatible subject by PCA distance, ties by subject index); the owner rejected a signature-*distance* metric as unnecessary machinery, so structure is an ordering, not a score. A route arm below the floor is dropped from the anchor pool with a loud alert, since no legal donor exists. `max_donor_weight` 0.80 -> **0.50** (landed at 0.30, revised 2026-07-27 on measurement: see `METHOD_DISCUSSION.md` §6a A2 "Choosing the cap" -- uncapped, one donor takes a median 58% of an avatar and the effective donor count is 2.37 of 5, so a cap is needed; but 0.30 fired on 99% of subjects, making it the weighting scheme rather than a guardrail, while 0.50 fires on ~68% and states as one sentence, no patient is more than half of any avatar), exposed as an argument, and enforced on *every* donor by water-filling (`.cap_weights()`) -- capping only `which.max()` left the runner-up above the stated maximum once the excess was redistributed. Measured on `theo_md`, the tightening is nearly free: effective donors 2.5 -> 3.9 while between-subject SD of log AUC is flat within noise (the independence prediction of a 81%->49% fall is wrong because donors are correlated neighbours). `pmx_settings` now records `max_donor_weight`, `cap_binding_fraction`, `mean_effective_donors`, `min_effective_donors`, and `routes` -- the binding fraction being the diagnostic that says whether a cap is acting as a guardrail or as the algorithm. A route arm that cannot reach the floor is handled by the new `on_donor_shortfall` argument -- `"drop"` (default, omit the arm), `"noise"` (keep it on whatever same-route donors exist plus noise; loudly marked not recommended, since such a subject can stay close to one real patient), `"error"` (refuse, naming both alternatives). Documented in `articles/avatar-algorithm.Rmd` Steps 6-7, which also now state outright that AMT is not a profile feature, so stage-2 blending does not preserve the dose-exposure relationship -- fine for code development, wrong for parameter estimation. Tests in `test-avatar-pooling.R` and `test-avatar-weights.R`. Still open: whether the headline floor should exceed 5 -- weakened, since the cap is now the tighter constraint. Linked to `REV-026`. | fixed |
| `REV-027` | AVATAR dose coherence and disclosure | Under dosing proportional to a baseline covariate (mg/kg, mg/m^2) an avatar's `AMT` was copied verbatim from its anchor while its covariates were blended from donors, so the avatar's own mg/kg was wrong: a cohort dosed at exactly 5 mg/kg produced avatars from 4.41 to 5.25, every one violating the protocol it claims to follow. Any weight-normalised analysis or data-consistency check sees it. The copied amount is also one real patient's real dose, which under proportional dosing discloses that patient's weight exactly -- the reason `skeleton_uniqueness()` reported nearly every subject unique on dose, and the same fact `REV-025` hit from the other end when weight-based dosing made every subject its own donor group. Found 2026-07-29. | **Fixed.** `.detect_dose_basis()` tests whether dividing `AMT` by each numeric covariate collapses the within-stratum variation; if so `.apply_dose_basis()` recomputes each avatar's amount as the stratum's multiplier times the avatar's *own* blended covariate, scaling any `rate` so the infusion duration is preserved. Both halves close: `AMT/WT` is exactly 5 for every avatar, and no generated dose is a source dose. Detection fails closed -- amounts must vary while ratios do not -- so `theo_md` (ratio CV 15%) and `wbcSim` are left alone while `warfarin` (1.5 mg/kg to within 0.1%) is recognised. Several dose levels are found by clustering the observed ratios rather than averaging within a declared group, so a 1/2/3 mg/kg escalation is recognised without the arm being declared -- and so is **intra-patient** escalation, where the level changes within a subject and no subject-constant grouping could ever see it. The guard against false positives is that a handful of ratios must stand in for many distinct amounts. This is **not** the dose-rescaling rejected in `REV-025`, which was about normalising donor trajectories before blending; this is about a synthetic patient's dose matching that same synthetic patient's weight. Recorded as `dose_basis` in `pmx_settings`; tests in `test-avatar-dose-attendance.R`. | fixed |
| `REV-029` | Blending had no measurement | Every masking mechanism had a measurement except the first. Donor blending protects the *values*, and `cap_binding_fraction` / `mean_effective_donors` describe how the blend was built rather than whether the result landed close to somebody real. `skeleton_uniqueness()` answers the structural question exactly; nothing answered the geometric one. Raised by the package owner as the original pairwise-distance idea, deferred while the skeleton defects were closed, and reinstated 2026-07-29. | **Fixed.** `compare_pmx_proximity()` reports a nearest-neighbour adversarial accuracy: for every subject, is its nearest neighbour in its own dataset or the other one? Near 0.5 is the target, toward 0 is memorisation, toward 1 is a utility failure. Raw distance-to-closest-record is deliberately not the headline -- it has no natural scale, and against zero it mostly tracks cohort size, so a larger source scores worse while blending across more donors is actually safer. The null is built by splitting the source cohort in half and running the identical statistic, so every small-sample artefact is present in null and observed alike and cancels; it is wide at pharmacometric cohort sizes, and the print method says so. `test-proximity.R` leads with a positive control -- a verbatim copy must be rejected -- because a privacy metric never seen to fire is an untested branch. | fixed |
| `REV-031` | Inferred grid, merge vs snap | `.derive_time_grid()` merges pooled times into visit cells under an explicit constraint -- a cell must never hold one subject twice, since collapsing two of a subject's own visits destroys a real visit rather than hiding a schedule -- and the comments stated that guarantee. But `.snap_to_grid()` then assigned each time to the **nearest cell centre**, a separate decision that knew nothing about cell membership, so a time near a boundary could land in a neighbouring cell and undo it. On `nimoData`, 35 of 321 observations ended up sharing a time with another observation of the same subject; example, subject 1's 1512.11 and 1512.90 both snapping to 1513.09. Found 2026-07-30 while writing `scripts/explore_nimodata_coarsening.qmd`. **Initially assessed as harmless and that was wrong**: a doubled visit becomes its own attendance cell, enters the pool, and can be drawn by any avatar, so the artefact is amplified rather than contained. | **Fixed.** `.coarsen_source_time()` now checks, after snapping, whether any subject's own visits landed on one time, and reverts those rows to their recorded times -- the outcome the merge had already decided on. Two things it deliberately does *not* touch. Rows already recorded at the same time were never distinct visits: `warfarin` subject 9 genuinely holds two `cp` values at times 3, 6, 9 and 12, and those stay. And a declared `nominal_time` legitimately puts two samples in one protocol slot, which is the user's statement about their own study; `mavoglurant`'s 800 same-(subject, endpoint, time) rows come from its reset clock, exist in the source as recorded, and are left alone. `nimoData` goes 35 to 0; every other public dataset is unchanged. Tests in `test-avatar-coarsen.R` cover both the repair and the two exemptions. | fixed |
| `REV-030` | Declared-but-unused roles | `pmx_roles()` accepts `addl` and `ii`, documents them as "optional additional-dose and interdose-interval columns", and nothing in the package reads either one — `grep` for `roles$addl` / `roles$ii` across `R/` returns nothing. They are retained by `.retained_role_columns()`, so the columns are carried into synthetic data verbatim from the anchor, and that is all. The failure mode is not the carry but the silence around it: a source encoding a QD regimen as one row with `ADDL = 27` presents to AVATAR as a **single-dose** subject. The event signature, the donor grouping, the dose-basis detection, and `skeleton_uniqueness()` all see one dose; the output then carries `ADDL = 27` beside a skeleton built as if there were one. Found 2026-07-30 while writing the README's full-declaration example. Same shape as `REV-020` (`rx` stored, never read). | **Open.** Options: (a) expand `ADDL`/`II` into explicit dose rows before profiling and re-collapse afterwards — correct, and the only version that makes the regimen visible to donor matching and to the coarsening grid; (b) refuse the roles in `synpmx_avatar()` with a message telling the user to expand first, as `.reject_dp_only_roles()` does for the DP-only roles — one line, honest, and it makes the limitation loud; (c) warn and carry. (b) now, (a) if a real study needs it: PIT565 and the oncology study will say which. Documented as a limitation in `README.md` in the interim. | open |
| `REV-028` | Remediation screening scope | `remediate_identifiable_subjects()` screens each replacement *batch* against itself, then appends, so a subject clean within its own batch can be an outlier once merged into the cohort. The documented contract is only that replacements are "screened by the same policy", so this is a gap between the contract and what a reader expects rather than a broken promise. Surfaced 2026-07-29 when attendance sampling made generated structure more uniform, tightening the DV spread enough that the robust median/MAD screen began flagging modest differences that previously sat inside the spread. | **Open.** Options: (a) re-screen the merged cohort after each append and continue replacing until clean, bounded by `max_tries` -- straightforward but can loop where the cohort cannot be made clean at all; (b) screen each candidate against the already-accepted cohort rather than against its batch; (c) document the batch-local scope explicitly and leave it. (b) is the smallest correct change. Test `test-remediate-identifiable.R` asserts the actual contract -- the flagged subject is gone and cohort size is restored -- rather than a globally clean result. | open |
| `REV-026` | AVATAR coherence / event skeleton | A synthetic subject inherits its anchor's event skeleton verbatim (`R/synthesis.R:531`, `skeleton <- source[profiles$subject_rows[[anchor]], ]`): the number and timing of its doses and observations equal one real subject's. So a patient with a unique schedule or long follow-up — e.g. the single long-followed subject in `wbcSim` — is a singleton group whose exact event structure reappears in the synthetic data. How many events a synthetic subject *should* have is never modeled; it is copied from one person. Found 2026-07-25. | **Open. Design in `design/METHOD_DISCUSSION.md` §6a (Phase 3).** Decouple the event skeleton from the value donors: sample a skeleton from the (relaxed) donor pool, or draw the observation/visit count from the cohort distribution within clinically plausible windows, rather than copying the anchor. Owner decision (2026-07-25): observations do **not** need dose support (PD and baseline observations exist without a nearby dose), so schedule sampling is freer than first scoped; the remaining constraint is that dosing events stay coherent with the regimen. Connects to protocol structure (`vignettes/articles/data-elicitation.Rmd`). Depends on the relaxed pooling from `REV-025`. **Partly done 2026-07-29.** `coarsen_time` made the observation half of this far cheaper than when it was parked: once every subject sits on a shared visit grid, a schedule reduces to a bitmap over known cells, and sampling a bitmap is a different problem from generating a schedule from nothing. `min_pattern_share` (default 3) now draws each avatar's attendance pattern from ones at least that many subjects share, so no one-of-a-kind pattern is reproduced and -- unlike dropping the exposed subjects -- nobody leaves the cohort. Still open: the **dose** half. Dose events are deliberately untouched, because sampling them can emit a regimen no protocol permits, which is the objection that ruled out randomized dose dropping under fixed escalation. **Cost addressed 2026-07-29** by drawing a *shape* before an arrangement. Matching exact patterns alone discarded nearly everything, because two patients who each missed one visit are different patterns when they missed different visits; as shapes (miss count plus terminal/contiguous/scattered) they are one group. A real pattern of the drawn shape is reused where one clears the floor, and only otherwise is an arrangement generated -- rejected and redrawn if it lands on a pattern too rare to reuse, so the output guarantee is not quietly downgraded to "nothing was copied on purpose". On `warfarin` the loss falls from 12 patterns to 2 and 5 of the source's 6 distinct sample counts survive; across the public sets the losses go 12/17/12/54 to 2/4/2/1. What remains lost is resolution: how much missingness and of what kind survive, which specific visits does not. | open |
