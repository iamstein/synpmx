...
The measurement also uses a fixture generated by the same structural model that
is later fitted, so it isolates the mechanism but flatters the model. Real data
is not generated by a one-compartment model, and structural misspecification is
not represented anywhere in these numbers.
