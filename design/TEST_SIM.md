...
The first evaluation set contains every dataset used by the evaluation
vignette (`vignettes/public-data-examples.Rmd`).
...
| `nimoData` | `nlmixr2data::nimoData` | Public package data | Four nominal dose groups, ten approximately weekly infusions, declared OCC/TAD, long terminal follow-up, and a time-varying weight column | DOS is a stratum that conditions amount/rate/duration; every subject has ten coherent infusions; WGT is explicitly excluded; dose-relative sample count and terminal coverage remain broad |
...
| `pheno_sd` | `nlmixr2data::pheno_sd` | Public package data | 59 **real** patients from routine clinical care rather than a protocol -- the least protocol-like source available. Neonatal phenobarbital: individualised dosing, sparse irregular sampling, and a time-varying weight | Both guarantees hold at 0 since `SIM-048`, and that is the point of keeping it: 55 of 59 patients have a dose schedule nobody shares (all 59 after coarsening), so re-anchoring reaches 0 only by building on the few who can be truncated to a shared depth -- and the one shared depth here is a **single dose**. Doses per patient go 10.0 -> 1.2 and 3 of 56 regimens are represented. This is the registry's example of a study whose dosing survives the guarantee in name only: B1a and B1b pass, A5 is where it shows |
...
| `pheno_sd` | nlmixr2data | 59 | 744 | 1 | **Real, and not from a protocol** -- routine neonatal clinical care: individualised dosing, sparse irregular sampling, time-varying weight. 55 of 59 dose schedules are held by one patient and cannot be masked, and the run says so | **in use** |
...
| `nmtest` | nlmixr2data | 54 | 7157 | 1 | NONMEM 7.4.3 event-grammar torture test: steady state, `ADDL`, `II`, lag time, bioavailability, duration and rate modes together. **The hardest available exercise of role handling**, and the only thing that would say whether the `addl`/`ii` carry-through is sound | candidate, highest value |
...
| `Oral_1CPT` and 11 siblings | nlmixr2data | 120 | 7920 | 1 | ACOP 2016 simulated sets: `Bolus`/`Infusion`/`Oral` x `1CPT`/`2CPT` x with/without Michaelis--Menten. The only public sources with `SS`/`II` populated at scale (no `ADDL` column; `nmtest` is the one with all three) | candidate, but see below |
...
are the only public sources with `SS`/`II` populated at scale and would
...
| `SIM-024` | Cohort assignments | `ACTARM`, `TRT`, or nominal dose group could be sampled independently of the generated regimen | Ordinary categorical covariates retained only marginal proportions and had no link to event summaries | Declare subject-level assignment fields as `strata`; release stratum count and regimen jointly; require generated property-to-event coherence |
...
| `SIM-049` | AVATAR re-anchoring pool | The `SIM-048` fix did not close it. On the owner's rerun of the same 52-patient 8-arm study, B1a read 1 and B1b read 8 -- still FAIL, still with `strata_crossed = 0`. Found 2026-08-13 by the package owner, from his own scorecard, on the version carrying the `SIM-048` fix | The safe set was ONE flag: dose schedule shared AND visit set shared. On a study where nearly every visit set is unique -- staggered visits, a missed sample here and there -- that flag is FALSE for the whole cohort, so `.reanchor_candidates()` found nothing to prefer and fell through to "anyone in the arm", dropping the dose condition along with the visit condition. An arm whose every dose schedule is its patient's own was then drawn from again, exactly as in `SIM-048`, but now invisibly, because the flag that fix turned on had gone empty. Reproduced on a 48-patient 8-arm fixture with unique visit sets throughout and one all-unique-dosing arm: all six of that arm's avatars leaked, with `strata_crossed = 0` | Fixed 2026-08-13, in two parts. (1) The safe set is a LIST of preference tiers -- both-safe, then dose-safe, then visit-safe -- so an empty top tier no longer discards the condition that still matters. Dose outranks visits because a dose schedule is copied verbatim and can never be masked, while a rare visit set usually has a substitute. (2) **Re-anchoring never leaves the avatar's arm**, which is the owner's decision on being shown the trade: an arm that has silently borrowed patients from another arm is a study that never ran and the reader cannot see it, while a disclosure the scorecard names is something they can act on. `strata_crossed` goes with it. Where an arm cannot mask its own avatars the run alerts and names the arm -- a new alert on the dose side, which could not previously happen -- and B1a/B1b read `6 in ARM8 (6)` rather than `6`. `unmaskable_strata(source, roles)` answers the same question from the source before generating, one row per arm, splitting unmaskable dosing from unmaskable visits. `tests/testthat/test-reanchor-strata.R` pins the tier fall-through, the arm-named leak, and that every arm keeps its source size |
| `SIM-048` | AVATAR re-anchoring pool | Six avatars carrying a visit set nobody else shares and seven carrying a dose schedule nobody else shares, on a 52-patient 8-arm study -- `identifying_visit_sets` and `identifying_dose_schedules` are the two numbers the guarantees are stated in terms of, so both scorecard B1 rows read FAIL. `strata_crossed` was 0, reporting that nothing had been given up to get there. Found 2026-08-13 by the package owner, from his own scorecard | A regression from `af72e77`, which confined re-anchoring to the avatar's own arm to stop the swap scrambling arm sizes. The fallback to the whole pool fires when the stratum has "nobody else to offer", and that was implemented as nobody *at all* rather than nobody who can be **masked**: an arm in which every dose schedule is unique has plenty of members, so the move stayed inside it, drew another unmaskable anchor, and after twelve tries emitted one anyway. Before `af72e77` the draw was from the whole pool and the same twelve tries almost always found somebody safe, which is why the guarantees had held. Two smaller faults in the same loop: candidates were rejection-sampled at random rather than filtered, so being safe was a coin toss even where safe anchors were plentiful; and a twelfth-attempt move fell out of the loop with the visit set that had been drawn for the anchor it had just left | Fixed 2026-08-13. `.reanchor_candidates()` takes the safe set and prefers safe-in-arm, then safe-anywhere (counted as `strata_crossed`), then in-arm, then anyone -- the last two being the cohort where nobody can be masked, which crossing cannot fix, so the arm sizes are kept rather than spent for nothing. **Superseded the same day by `SIM-049`**, which found the leak still open on the owner's data and removed crossing altogether: the arm is never left, and a leak is reported against the arm that caused it. Safe means: dose schedule shared, and visit set shared. The loop stops on an anchor whose visit set was actually drawn for it. On a 24-patient two-arm fixture whose second arm doses every patient on their own days, all twelve of its avatars leaked before and none do after, with the crossing now reported. `pheno_sd` goes from 14-21 leaked dose schedules to 0 -- see its registry row for what that costs. `tests/testthat/test-reanchor-strata.R` pins the preference order and recomputes the end-to-end count from the finished table |
...
| `SIM-046` | AVATAR donor interpolation | A donor's trajectory was extrapolated outside the times that donor was actually observed at, by rescaling the anchor's time range onto the donor's own -- "the same shape on a stretched clock". That is wrong whenever the missing region is a distinct kinetic phase rather than a shifted window. On `warfarin`, 19 of 32 subjects have no `cp` sample before 24 h, so a donor's 24 h **elimination** concentration was served up as an anchor's 0.5 h **absorption** concentration: generated median at 0.5 h was 8.76 against a source median of 0.00, the absorption limb was replaced by a flat plateau, Cmax spread fell from 6.6-17.6 to 5.3-12.9, and the warped segment rejoined the real data with a visible upward step at 24 h (12 h median 8.05, 24 h median 9.07). The `pca` baseline widened the same way, 82-100 in the source coming back 45-126. Every scorecard row passes -- nothing scores profile shape. Found 2026-08-13 by the package owner, reading the `warfarin` figure in `public-data-examples.Rmd` | Present since the initial implementation (`git log -S target_fraction`), never covered by a test. `.interpolate_trajectory()` used `approx(rule = 1)`, which correctly yields NA outside the donor's range, and then filled every NA by mapping the target's position within its own range onto the donor's range. Two separate claims were conflated: interpolating *between* a donor's observations is a statement about a patient measured either side of the gap; extending *beyond* them is a statement about a patient nobody measured | Fixed 2026-08-13. `.interpolate_trajectory()` returns NA outside the donor's observed range and the warp is gone; `.weighted_available()` already blends over the donors that do cover each row and renormalizes their weights, so no new machinery was needed there. A row **no** donor covers had been falling to a dataset-wide median -- the one quantity guaranteed to have the wrong shape, putting 6.4 at `warfarin`'s 0.5 h -- so `.cohort_median_trajectory()` replaces it with a median **per time**, over times at least `min_pattern_share` subjects were observed at, interpolated with `rule = 2`. A median over one subject is that subject, which is why the floor is there. Generated `cp` medians now track the source across the whole profile (0.00 at 0.5 h, 8.12 at 2 h, 11.88 at 6 h, 9.07 at 24 h) and Cmax median matches at 10.3 against 10.1. `tests/testthat/test-avatar.R` pins that a donor observed only from 24 h returns NA below it rather than its 24 h value |
| `SIM-047` | AVATAR generated follow-up depth | An avatar was emitted with two observations, at 501 h and 3910 h, on `wbcSim` -- a follow-up length no real patient of its shape has, and a fresh signal that *somebody* was followed to 3910 h. Only one source subject reaches that far. Found 2026-08-13 by the package owner, reading the `wbcSim` figure in `public-data-examples.Rmd` | Not the anchor screen's business: `screen = TRUE` removes long-follow-up subjects from the **anchor** pool, and this avatar's depth came from the **attendance** draw, which the screen never looks at. `.attendance_cells()` builds the grid as the union over the group -- 23 cells for `wbcSim`, 11 of them held only by the single subject followed to 4580 h -- and `.attendance_shape()` reduces a pattern to kind + count, discarding where on the timeline it sat. `trailing` and `block` both leave the tail attended so neither can overshoot; `scattered` is free to hold any cells at all. A two-visit `scattered` shape was placed on cells 7 and 19 of 23. B1a and B4a cannot see it: both ask whether a real patient's set was **copied**, and `.place_attendance()` explicitly rejects any placement matching a rare real pattern, so a fabricated schedule is exactly what passes. `flag_identifiable_subjects()` did flag the subject on follow-up time and scorecard B2 read `12 of 45 review`, but `wbcSim`'s follow-up is bimodal so it sat in a list beside avatars at 336 h and read as routine | Fixed 2026-08-13. `.shared_depth()` returns the last grid cell at least `min_pattern_share` subjects were still being observed at (cell 13, 1705 h, on `wbcSim`), and `.place_attendance()` treats droppable cells beyond it as missed outright rather than as candidates, arranging the remainder over what is left -- which holds the avatar's visit **count** unchanged and only moves the visits inside the study's shared follow-up. Non-droppable cells past the cap stay attended, since a complete endpoint is complete by construction. Deepest `wbcSim` avatar falls from 3910 h to 958 h. `tests/testthat/test-avatar-dose-attendance.R` builds the `wbcSim` shape deliberately -- two subjects holding different two-visit keys but the same `scattered` shape, so the shape clears the sharing floor and a placement is generated for it -- and pins that no avatar passes the shared depth; with the cap removed the same fixture puts 4 of 30 avatars past it, one at the depth a single patient holds. A shape whose every holder is unique is dropped from the pool and never reaches placement at all, which is why the obvious one-rare-subject fixture does not reproduce it. Also fixed here: `.attendance_key_from(cells, integer(0))` returned the empty key, because `cells[-integer(0)]` is `cells[0]` |
| `SIM-045` | AVATAR discrete endpoints | `xgxr::mad` carries binary, count and ordinal PD endpoints in the same numeric `LIDV` column as PK and continuous PD, and all three come back continuous: the 0/1 binary endpoint takes 600 distinct values spanning -0.13 to 1.08, the 1/2/3 ordinal takes 600 spanning 0.53 to 4.69, and the integer count takes 599. Every scorecard row passes and `compare_pmx_distributions()` reports plausible means, so nothing flags it. Found 2026-08-13 while rewriting `public-data-examples.Rmd` | Class restoration is per **column**, and a PMX table keeps every endpoint in one numeric DV column, so there is nothing per-endpoint to restore. Blending k donors' values is a weighted mean, which already leaves an integer level set, and the subject and residual noise terms then carry it outside the source range entirely | Fixed 2026-08-13. `.endpoint_value_types()` decides per endpoint, from the source, whether it is `binary` (every value 0 or 1), `ordinal` (whole numbers, at most 12 levels), `integer` (whole numbers, more levels) or `continuous`, and `.snap_endpoint_values()` puts each generated value back on that scale at emit, before censoring reads it. Inference is on by default -- unlike the dose basis, the question is answered outright by the data and the repair reproduces granularity the source already had -- and `pmx_roles(endpoint_types = )` overrides it either way, refusing an impossible declaration (`binary` on a 53-level endpoint) rather than widening. `synpmx_avatar()` recomputes the violation count from the finished table into `endpoint_type_violations`, and scorecard row A6 asks it of any synthetic data. `tests/testthat/test-endpoint-types.R` pins the detector, the emitted values, the override in both directions, that a continuous endpoint is NOT quantized as a side effect, and -- by declaring the discrete endpoints continuous -- that the original defect still reproduces. Known consequence: `warfarin`'s `pca`, a percentage recorded without decimals, is now rounded to whole numbers like its source |
...
| `SIM-050` | AVATAR time coarsening / dose grid | `nimoData` is 12 of 12 identifying purely because dose times are recorded actuals. Doses are weekly, but 165.70 / 167.24 / 168.05 land in three cells, and 86 of 97 dose times after coarsening are held by one patient. Raised 2026-08-03; carried unresolved out of `design/TODO.md` when that file was deleted 2026-08-14 | A dose-only grid merges them perfectly (104 distinct times collapse to 10 weekly cells) and every attempt below takes `nimoData` to 0. What none survives is what happens to the observations around a dose that has just moved. **Three attempts, none shippable.** (1) *Independent observation grid* -- an observation at 167.16 snaps to 167.10 while its dose at 167.20 snaps to 166.73, so the dose lands before the sample that preceded it; fires on all 12 subjects, and guarded per subject it reverts all 12 and is a no-op everywhere. (2) *Clamp observations between neighbouring dose cells* -- measured strictly better on all five public sets (`nimoData` dose 12 to 0, `wbcSim` observation-unique 17 to 13, nothing worse), but it drags every post-dose sample forward onto the dose when the dose moves far, collapsing three real visits into one; the spurious-collision repair then reverts those rows, leaving a mixed state that can itself reorder. Passes the public numbers, fails a dense-PK fixture. (3) *Carry samples with their dose, preserving time-after-dose* -- conceptually right and what the owner described; fixes `nimoData` and produces identical schedules on a dense-PK fixture, but regresses `theo_md` from 0 to 12 unique observation schedules because the elapsed-time axis is unstable at interval boundaries (a trough a minute before a dose is 23.9 h after the previous one; the same trough a minute after is 0.02 h after this one). Referring each sample to the NEAREST dose rather than the preceding one was tried and does not fix it | **Open.** Attempt 3 is the one to finish; the remaining problem is entirely the boundary, and it needs the elapsed-time grid derived per dose interval, or a reference rule that cannot straddle. The alternative not yet attempted is making **doses authoritative**: derive the dose grid first, then clamp each observation into the interval between its neighbouring snapped dose times, so a pre-dose sample ties with its dose rather than crossing it, keeping dose cells shared. It touches coarsening for every dataset. Whatever is tried must be measured on all five public sets plus `case1_pkpd` and against the `SIM-014` timing gates before it ships -- attempt 2 is the proof that passing the public numbers is not sufficient evidence |
| `SIM-051` | AVATAR dose fidelity reporting | Reaching B1b = 0 by truncating a ten-dose regimen to a single dose is a scorecard pass that should not read as one. `pheno_sd` after `SIM-048` is the worked case: both B1 rows pass, while doses per patient go 10.0 -> 1.2 and 3 of 56 regimens are represented. Raised 2026-08-13 alongside `SIM-048`; carried out of `design/TODO.md` when that file was deleted 2026-08-14 | Dose-count fidelity has no check of its own. Scorecard row A5 shows doses per patient, but nothing states "this study's dosing did not survive" in a unit a pharmacometrician acts on, and `dose_regimens_represented` exists in `pmx_masking_report()` and in no scorecard row | **Open.** Give dose fidelity a scorecard row of its own, computed from the finished table, so a study whose dosing was truncated to reach the B1 guarantee fails visibly rather than passing quietly. This is the reason `pheno_sd` is kept in the dataset registry |
...
- Privacy concepts and the simulation algorithm were initially mixed in one
  ...
  simulation-method vignettes, plus the evaluation vignette.
...
- `strata_summary()` contains public DOS strata 50, 100, 200, and
