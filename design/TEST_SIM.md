...
| `SIM-034` | AVATAR timing, inferred grid | Coarsening can silently fail to collapse anything | With no `nominal_time` role the grid is inferred, and the largest cut that preserves every subject's own visit count can be zero when subjects hold samples closer together than the spread between subjects. Correct — no grid can merge those without destroying a real visit — but silent. `scripts/measure_skeleton_uniqueness.R` shows it plainly: warfarin and mavoglurant do not move at all, theo_md goes 12/12 to 5/12 | `synpmx_avatar()` counts subjects still alone after coarsening and raises a loud alert naming the count and pointing at the `nominal_time` role; `skeleton_uniqueness()` reports the classes separately so the cause is visible |
