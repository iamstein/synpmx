...
synpmx_pca_summarize <- function(data, roles, seed = NULL,
  ...
  arm_models <- .arm_models(source, roles, .pca_cells(fit), subject_group,
                            min_arm_patients)
...
# The grid `.arm_models()` measures attendance on, read off a fitted basis.
# The shared model takes the cells rather than the fit, so this is the only
# place the two representations meet.
.pca_cells <- function(fit) {
  index <- which(fit$kinds == "endpoint_cell")
  data.frame(
    index = unname(index),
    name = names(index),
    endpoint = unname(vapply(fit$members[index],
                             function(m) as.character(m$endpoint),
                             character(1))),
    time = unname(vapply(fit$members[index], function(m) as.numeric(m$time),
                         numeric(1))),
    row.names = NULL,
...
.pca_generate <- function(trial_summary, n_subjects) {
  ...
  schedules <- lapply(assignment, function(arm) {
    .draw_schedule(trial_summary$dosing[[arm]])
  })
